...
This guide covers the complete development workflow for the Matrix theme, from environment setup to building production packages.
...
git clone git@github.com:eamonxg/luci-theme-matrix.git
cd luci-theme-matrix/.dev/
...
3. CSS requests to `/luci-static/matrix/main.css` and `/luci-static/matrix/login.css` are rewritten to serve from `.dev/src/media/main.css` and `.dev/src/media/login.css` respectively
...
2. **Build splits, not bundles.** `vite.config.ts` adds every `media/patches/*.css` as its own Rollup entry, so each compiles to `htdocs/luci-static/matrix/patches/<page>.css`. They are no longer part of `main.css`.
...
4. Verify `htdocs/luci-static/matrix/patches/<page>.css` is small (just your rules, not a copy of `main.css`).
...
└── resources/
    └── menu-matrix.js     # Menu configuration (minified via Terser)
```
...
5. Static assets copied from `.dev/public/matrix/`
...
│   ├── public/matrix/              # Public static assets
│   │   ├── fonts/                  # Web fonts (Lato)
...
│       └── menu-matrix.js          # Minified menu logic
├── root/etc/uci-defaults/          # OpenWrt system integration
│   └── 30_luci-theme-matrix        # Theme auto-setup script
├── ucode/template/themes/matrix/   # LuCI ucode templates
